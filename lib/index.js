...
class IconsWebpackPlugin extends MemoryWebpackPlugin {
    ...
    generateOptions(context, options) {
        ...
        if (!options || typeof options !== 'object') {
            throw new Error(`[IconsWebpackPlugin] ${ options } is not a object.`);
        }

        // 校验入口文件
        if (typeof options.input !== 'string') {
            throw new Error(`'[IconsWebpackPlugin] ${ options.input }' is not a string.`);
        }
        ...
        if (!(fs.isDirectory(options.input))) {
            throw new Error(`'[IconsWebpackPlugin] ${ options.input }' is not a directory.`);
        }
